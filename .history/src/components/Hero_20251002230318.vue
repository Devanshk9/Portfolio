...
<template>
  ...
  <div class="flex flex-col md:mt-4 items-center justify-center overflow-hidden min-h-screen text-white space-y-4">
    ...
    <div v-for="(rail, i) in rails" :key="i" class="w-full overflow-hidden whitespace-nowrap">
      ...
      >
        ...
        <p class="px-8 text-4xl" :class="rail.cls">
          <!-- escape <= to avoid Vue parsing issues -->
          {{ rail.text.replaceAll('≤','\u2264') }}
        </p>
        <p class="px-8 text-4xl" :class="rail.cls" aria-hidden="true">
          {{ rail.text.replaceAll('≤','\u2264') }}
        </p>
        <p class="px-8 text-4xl" :class="rail.cls" aria-hidden="true">
          {{ rail.text.replaceAll('≤','\u2264') }}
